...
business_file_path = "C:/Users/achou/Downloads/R/project/yelp_academic_dataset_business.json"
...
```
From the above plot,  we notice that there isnt a drastic difference in the average rating of the different clusters. Rather, we notice that the average tends to be higer in less populated areas. This is due to the sparse density of data points in the region.
We can conclude that, even if someone has a business on the Las Vegas Strip, doesnt really effect the ratings if the service isnt good. Hence, location plays a not so important role in the ratings a business gets.